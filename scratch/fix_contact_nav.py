...
nav_end_idx = content.find('</header>')
last_div_idx = content.rfind('</div>', 0, nav_end_idx)
if last_div_idx != -1:
    content = content[:last_div_idx] + hamburger_btn + content[last_div_idx:]

# Re-find </header> after insertion
nav_end_idx = content.find('</header>')

# Inject mobile menu HTML + script right after </header>
content = content[:nav_end_idx + 9] + '\n' + mobile_menu_html + content[nav_end_idx + 9:]
